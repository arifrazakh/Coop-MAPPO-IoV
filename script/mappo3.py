...
4. controlled no-neighbor observation ablation,
5. independent training seeds, and
6. urban/highway/mixed scenario breakdown, and
7. matched MA-A2C / MA-SAC baseline reruns on the same revised environment.
...
CODE_VERSION = "2026-08-23-targeted-v5-sac-uniform-replay-fix"
...
from ray.rllib.algorithms.sac import SACConfig   # <<< MA-SAC baseline
...
class SharedA2CNet(nn.Module):
    """Shared local-observation actor/value network for the MA-A2C baseline.

    Ray 2.49 removed the old rllib_contrib algorithms, including the historical
    RLlib A2C implementation.  To keep the baseline runnable in the same modern
    environment as PPO/MAPPO/SAC, this small in-script A2C implementation uses
    the exact same 20-D observation, 4-D bounded action, common reward, and one
    parameter-shared actor/value function across BSs.
    """

    def __init__(self, obs_dim=LOCAL_OBS_DIM, action_dim=4, hidden=(256, 256)):
        super().__init__()
        layers = []
        last = int(obs_dim)
        for h in hidden:
            layers.extend([nn.Linear(last, int(h)), nn.Tanh()])
            last = int(h)
        self.body = nn.Sequential(*layers)
        self.actor_mean = nn.Linear(last, int(action_dim))
        self.value_head = nn.Linear(last, 1)
        self.log_std = nn.Parameter(torch.full((int(action_dim),), -0.5))

    def forward(self, obs_t):
        z = self.body(obs_t)
        return self.actor_mean(z), self.value_head(z).squeeze(-1)
    ...
    def sample(self, obs_t, deterministic=False):
        ...
        # Density correction for tanh and affine map y -> (y+1)/2.
        logp = dist.log_prob(pre_tanh)
        logp = logp - torch.log(1.0 - y.pow(2) + 1e-6) + math.log(2.0)
        logp = logp.sum(dim=-1)
        return action, logp, value

    def evaluate_actions(self, obs_t, action_t):
        mean, value = self.forward(obs_t)
        log_std = torch.clamp(self.log_std, -5.0, 2.0).expand_as(mean)
        std = torch.exp(log_std)
        dist = torch.distributions.Normal(mean, std)

        y = torch.clamp(2.0 * action_t - 1.0, -0.999999, 0.999999)
        pre_tanh = self._atanh(y)
        logp = dist.log_prob(pre_tanh)
        logp = logp - torch.log(1.0 - y.pow(2) + 1e-6) + math.log(2.0)
        logp = logp.sum(dim=-1)

        # A sample-based entropy estimate is sufficient for entropy regularization
        # under the squashed Gaussian policy.
        entropy_est = -logp
        return logp, entropy_est, value


class SharedA2CAlgorithm:
    """Minimal synchronous parameter-shared MA-A2C adapter.

    It intentionally exposes the small subset of the RLlib Algorithm interface
    used by this experiment runner: train(), compute_single_action(),
    save_to_path(), and stop().  This avoids forcing the entire project onto an
    obsolete Ray version merely to recover A2C.
    """

    def __init__(self, spec: ExperimentSpec, args, env_cfg: dict):
        self.spec = spec
        self.args = args
        self.env_cfg = dict(env_cfg)
        self.env = rllib_env_creator(self.env_cfg)
        self.agents = list(self.env.agents)

        use_cuda = float(args.num_gpus) > 0 and torch.cuda.is_available()
        ...
        self.gamma = float(args.a2c_gamma)
        self.gae_lambda = float(args.a2c_lambda)
        self.vf_coeff = float(args.a2c_vf_coeff)
        self.entropy_coeff = float(args.a2c_entropy_coeff)
        self.grad_clip = float(args.a2c_grad_clip)
        self.epochs = max(1, int(args.a2c_epochs))
        self.minibatch_size = max(1, int(args.a2c_minibatch_size))

        # Match the PPO runner's complete-episode geometry.  For the paper
        # defaults (1024 requested env steps, 200-step episodes), one A2C train()
        # call collects 6 complete episodes = 1200 joint environment steps.
        requested = max(1, int(args.train_batch_size))
        ep_len = max(1, int(args.episode_len))
        self.episodes_per_iteration = max(1, int(math.ceil(requested / ep_len)))
    ...
    def _episode_seed(self):
        # Deterministic but distinct training reset seed for every episode.
        return int(self.spec.seed) * 1_000_000 + int(self.total_episodes)

    def _write_training_episode(self, episode_len, episode_return, scalar_history):
        path = Path(os.environ.get("IOV_TRAIN_EPISODE_CSV", ""))
        if not str(path):
            return
        row = {
        ...
        for k, vals in scalar_history.items():
            if vals:
                row[k] = float(np.mean(vals))
        append_dict_row(path, row)
    ...
    def train(self):
        ...
        for _ in range(self.episodes_per_iteration):
            ...
            for _step in range(int(self.args.episode_len)):
                obs_np = np.stack(
                    [np.asarray(obs[a], dtype=np.float32)[:LOCAL_OBS_DIM] for a in self.agents],
                    axis=0,
                )
                obs_t = torch.as_tensor(obs_np, dtype=torch.float32, device=self.device)
                with torch.no_grad():
                    action_t, _logp_t, value_t = self.model.sample(obs_t, deterministic=False)

                actions_np = action_t.cpu().numpy().astype(np.float32)
                action_dict = {
                    agent: np.clip(actions_np[i], 0.0, 1.0)
                    for i, agent in enumerate(self.agents)
                }

                next_obs, rewards, terminateds, truncateds, infos = self.env.step(action_dict)
                done = bool(
                    terminateds.get("__all__", False)
                    or truncateds.get("__all__", False)
                )
                common_reward = float(rewards[self.agents[0]])

                for i, agent in enumerate(self.agents):
                    tr = traj[agent]
                    tr["obs"].append(obs_np[i])
                    tr["actions"].append(action_dict[agent])
                    tr["rewards"].append(common_reward)
                    tr["values"].append(float(value_t[i].item()))
                    tr["dones"].append(done)

                info0 = infos.get(self.agents[0], {}) if isinstance(infos, dict) else {}
                for k, v in info0.items():
                    if isinstance(v, (int, float, np.integer, np.floating)):
                        scalar_history[k].append(float(v))

                ep_return += common_reward
                ep_len += 1
            ...
            # Complete episodes => terminal bootstrap value is zero.
            for agent in self.agents:
                tr = traj[agent]
                n = len(tr["rewards"])
                ...
                for t in range(n - 1, -1, -1):
                    nonterminal = 0.0 if tr["dones"][t] else 1.0
                    next_value = (
                        tr["values"][t + 1]
                        if (t + 1) < n
                        else 0.0
                    ...
                    adv[t] = gae
                    ret[t] = gae + tr["values"][t]

                batch_obs.extend(tr["obs"])
                batch_actions.extend(tr["actions"])
        ...
        adv_t = torch.as_tensor(
            np.asarray(batch_adv, dtype=np.float32),
            dtype=torch.float32,
            device=self.device,
        )
        ...
        for _epoch in range(self.epochs):
            ...
            for start in range(0, n_samples, self.minibatch_size):
                ...
                logp, entropy_est, values = self.model.evaluate_actions(
                    obs_t[idx],
                    act_t[idx],
                )
                policy_loss = -(logp * adv_t[idx].detach()).mean()
                value_loss = torch.mean((values - ret_t[idx]) ** 2)
                entropy = entropy_est.mean()
                loss = (
                    policy_loss
        ...
        return {
            "num_env_steps_sampled_lifetime": int(self.total_env_steps),
            "episode_return_mean": (
            ...
            "custom_a2c_loss": self.last_loss,
        }

    def compute_single_action(self, obs, explore=False, policy_id=None, **kwargs):
        obs_np = np.asarray(obs, dtype=np.float32).reshape(-1)[:LOCAL_OBS_DIM]
        obs_t = torch.as_tensor(
            obs_np[None, :],
            dtype=torch.float32,
            device=self.device,
        ...
        with torch.no_grad():
            action, _logp, _value = self.model.sample(
                obs_t,
                deterministic=not bool(explore),
            )
    ...
    def save_to_path(self, path):
        ...
        ckpt = out_dir / "a2c_checkpoint.pt"
        torch.save(
            {
...
def _configure_rllib_rollouts(
    ...
    *,
    ...
    """Version-tolerant old-stack rollout configuration."""
    configured = False
    try:
        config.env_runners(
    ...
    if not configured:
        try:
            config.rollouts(
                num_rollout_workers=int(num_env_runners),
                rollout_fragment_length=rollout_fragment_length,
                batch_mode=str(batch_mode),
            )
            configured = True
        ...
        if hasattr(config, "sample_timeout_s"):
            config.sample_timeout_s = float(sample_timeout_s)
    return config


def _configure_common_rllib(config, spec: ExperimentSpec, args, env_id: str, env_cfg: dict):
...
def _build_ppo_family_algorithm(spec: ExperimentSpec, args, env_cfg: dict):
    config = PPOConfig()
    env_id = CENTRAL_ENV_ID if spec.method == "cent_ppo" else MULTI_ENV_ID
    _configure_common_rllib(config, spec, args, env_id, env_cfg)
...
def _build_sac_algorithm(spec: ExperimentSpec, args, env_cfg: dict):
    """Build the shared-policy MA-SAC baseline on the same local observation/action."""
    config = SACConfig()
    _configure_common_rllib(config, spec, args, MULTI_ENV_ID, env_cfg)

    # SAC is off-policy and is naturally sampled in short fragments into a replay
    # buffer.  We therefore do not force PPO's complete-episode fragment geometry.
    sac_fragment = max(1, int(args.sac_rollout_fragment_length))
    _configure_rllib_rollouts(
        config,
        num_env_runners=int(args.num_env_runners),
        rollout_fragment_length=sac_fragment,
        batch_mode="truncate_episodes",
        sample_timeout_s=float(args.sample_timeout_s),
    )

    # Use uniform multi-agent replay on the old RLlib API stack.
    # The previous prioritized buffer could eventually produce invalid sampling
    # indices (after many training iterations) in Ray 2.49 multi-agent SAC.
    # Uniform replay is standard SAC behavior and avoids that failure without
    # changing the simulator, observation/action spaces, reward, networks, or
    # total environment-step budget.
    replay_cfg = {
        "_enable_replay_buffer_api": True,
        "type": "MultiAgentReplayBuffer",
        "capacity": int(args.sac_replay_capacity),
    ...
    config.training(
        gamma=float(args.sac_gamma),
        train_batch_size=int(args.sac_train_batch_size),
        twin_q=True,
        tau=float(args.sac_tau),
        initial_alpha=float(args.sac_initial_alpha),
        target_entropy="auto",
        n_step=1,
        replay_buffer_config=replay_cfg,
        num_steps_sampled_before_learning_starts=int(args.sac_learning_starts),
        optimization_config={
            # RLlib old-stack SAC consumes these keys from `optimization`.
            "actor_learning_rate": float(args.sac_actor_lr),
            "critic_learning_rate": float(args.sac_critic_lr),
            "entropy_learning_rate": float(args.sac_alpha_lr),
        },
        policy_model_config=policy_model_cfg,
        q_model_config=q_model_cfg,
        clip_actions=False,
    )
    ...
    # Reduce purely administrative train()/checkpoint frequency for long SAC
    # experiments while preserving the same overall environment-step budget.
    if hasattr(config, "min_sample_timesteps_per_iteration"):
        config.min_sample_timesteps_per_iteration = int(
            args.sac_min_sample_steps_per_iteration
        )
    if hasattr(config, "min_time_s_per_iteration"):
...
def build_algorithm(spec: ExperimentSpec, args):
    ...
    if spec.method == "ma_a2c":
        return SharedA2CAlgorithm(spec, args, env_cfg)
    if spec.method == "ma_sac":
        return _build_sac_algorithm(spec, args, env_cfg)
    if spec.method in {"ours", "mappo", "cent_ppo"}:
        return _build_ppo_family_algorithm(spec, args, env_cfg)
...
def build_paper_tables(all_summaries: Sequence[dict], output_dir: Path):
    summary_dir = output_dir / "summaries"
    summary_dir.mkdir(parents=True, exist_ok=True)
    rows = list(all_summaries)
    write_dict_rows(summary_dir / "run_summary.csv", rows)

    # Table: Cent-PPO vs centralized-critic MAPPO vs Ours, independent seeds.
    table1 = []
    for method in ["cent_ppo", "mappo", "ours"]:
        selected = [
            r for r in rows
        ...
        if selected:
            out = aggregate_group(selected, "method", method)
            table1.append(out)
    write_dict_rows(summary_dir / "table_mappo_seed.csv", table1)

    # Table: controlled neighbor-information ablation.
    table2 = []
    ...
    # Table: proposed method by mobility regime.
    table3 = []
    ...
    # Table: newly rerun MA-A2C and MA-SAC baselines across independent seeds.
    table4 = []
    for method in ["ma_a2c", "ma_sac"]:
        selected = [
            r for r in rows
            if r.get("method") == method
            and r.get("scenario") == "urban"
        ...
        if selected:
            table4.append(aggregate_group(selected, "method", method))
    write_dict_rows(summary_dir / "table_a2c_sac_seed.csv", table4)

    # If results from all methods are later gathered into one directory, this
    # becomes the complete cross-seed learning comparison automatically.
    table5 = []
    for method in ["ours", "mappo", "cent_ppo", "ma_a2c", "ma_sac"]:
        selected = [
            r for r in rows
            if r.get("method") == method
            and r.get("scenario") == "urban"
        ...
        if selected:
            table5.append(aggregate_group(selected, "method", method))
    write_dict_rows(summary_dir / "table_all_learning_seed.csv", table5)
...
def baseline_specs(seeds: Sequence[int], methods: Sequence[str]):
    """Only selected MA-A2C/MA-SAC reruns; other methods are not repeated."""
    allowed = {"ma_a2c", "ma_sac"}
    chosen = [str(m) for m in methods if str(m) in allowed]
    if not chosen:
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Targeted reviewer-requested PPO/MAPPO experiments for clustered IoV-VEC"
    )
    p.add_argument("--suite", choices=["targeted", "baselines", "single"], default="targeted")
    p.add_argument("--output-dir", default="targeted_experiment_results")
    ...
    p.add_argument(
        "--baseline-methods",
        nargs="+",
        choices=["ma_a2c", "ma_sac"],
        default=["ma_a2c", "ma_sac"],
        help=(
            "Methods to run when --suite baselines is selected. "
            "Use one method to split A2C and SAC across different servers."
        ),
    )
    ...
    p.add_argument(
        ...
        default=0,
        help=(
            "Sampling fragment length. 0 means use --episode-len, which is "
            "recommended with batch_mode=complete_episodes."
        ),
    )
    p.add_argument(
        "--sample-timeout-s",
    ...
    # MA-A2C baseline.  This is implemented locally because Ray 2.49 removed
    # historical rllib_contrib A2C.  The observation/action/reward/environment
    # remain identical to the other multi-agent methods.
    p.add_argument("--a2c-lr", type=float, default=5e-5)
    p.add_argument("--a2c-gamma", type=float, default=0.99)
    p.add_argument("--a2c-lambda", type=float, default=0.95)
    p.add_argument("--a2c-vf-coeff", type=float, default=0.5)
    p.add_argument("--a2c-entropy-coeff", type=float, default=0.01)
    p.add_argument("--a2c-grad-clip", type=float, default=0.5)
    p.add_argument("--a2c-epochs", type=int, default=1)
    p.add_argument("--a2c-minibatch-size", type=int, default=256)

    # MA-SAC baseline (RLlib old-stack SAC, shared policy over BS agents).
    p.add_argument("--sac-train-batch-size", type=int, default=256)
    p.add_argument("--sac-rollout-fragment-length", type=int, default=1)
    p.add_argument("--sac-learning-starts", type=int, default=1500)
    p.add_argument("--sac-replay-capacity", type=int, default=1_000_000)
    p.add_argument("--sac-gamma", type=float, default=0.99)
    p.add_argument("--sac-tau", type=float, default=5e-3)
    p.add_argument("--sac-initial-alpha", type=float, default=1.0)
    p.add_argument("--sac-actor-lr", type=float, default=3e-4)
    p.add_argument("--sac-critic-lr", type=float, default=3e-4)
    p.add_argument("--sac-alpha-lr", type=float, default=3e-4)
    p.add_argument(
        "--sac-min-sample-steps-per-iteration",
        type=int,
        default=1000,
        help=(
            "Reporting granularity for SAC. This changes how often train() returns "
            "and checkpoints/logs are emitted; it does not change the total "
            "--train-env-steps budget."
        ),
    )

    # Single-run convenience mode.
    p.add_argument("--method", choices=["ours", "mappo", "cent_ppo", "ma_a2c", "ma_sac"], default="ours")
...
def main():
    ...
    if int(args.a2c_minibatch_size) <= 0:
        raise ValueError("--a2c-minibatch-size must be > 0")
    if int(args.a2c_epochs) <= 0:
        raise ValueError("--a2c-epochs must be > 0")
    if int(args.sac_train_batch_size) <= 0:
        raise ValueError("--sac-train-batch-size must be > 0")
    if int(args.sac_rollout_fragment_length) <= 0:
        raise ValueError("--sac-rollout-fragment-length must be > 0")
    if int(args.sac_replay_capacity) <= 0:
        raise ValueError("--sac-replay-capacity must be > 0")
    if int(args.sac_min_sample_steps_per_iteration) <= 0:
        raise ValueError("--sac-min-sample-steps-per-iteration must be > 0")
    ...
    try:
        ...
        # Model may already be registered in an interactive/session rerun.
        pass

    if args.suite == "targeted":
        specs = targeted_specs(args.seeds)
    elif args.suite == "baselines":
        specs = baseline_specs(args.seeds, args.baseline_methods)
    else:
        specs = [
            ExperimentSpec(
                args.method,
    ...
    try:
        for spec in specs:
            ...
            # CSV-loaded summaries are strings when --skip-existing; normalize metadata.
            if isinstance(summary, dict):
                summaries.append(summary)
        build_paper_tables(summaries, output_dir)
    finally:
        ray.shutdown()
    ...
    print(f"  {output_dir / 'summaries' / 'table_neighbor_ablation.csv'}")
    print(f"  {output_dir / 'summaries' / 'table_mobility_breakdown.csv'}")
    print(f"  {output_dir / 'summaries' / 'table_a2c_sac_seed.csv'}")
    print(f"  {output_dir / 'summaries' / 'table_all_learning_seed.csv'}")
